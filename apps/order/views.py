...
from apps.order.permissions import HasCustomerAccessPermission
from apps.order.models import UserAddressModel
from apps.order.forms import CheckOutForm
from apps.cart.models import CartModel, CartItemModel
from apps.order.models import OrderModel, OrderItemModel
from django.urls import reverse_lazy
from apps.cart.cart import CartSession
from decimal import Decimal
from apps.order.models import CouponModel
from django.http import JsonResponse
from django.utils import timezone
from django.shortcuts import redirect
from apps.payment.zarinpal_client import ZarinPalSandbox
...
class OrderCheckOutView(LoginRequiredMixin, HasCustomerAccessPermission, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["total_price"] = '{:,}'.format(round(total_price))
        context["total_tax"] = '{:,}'.format(round((total_price * 9) / 100))
        return context
...
class ValidateCouponView(LoginRequiredMixin, HasCustomerAccessPermission, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        status_code = 200
        message = "discount code is applied."
        total_price = 0
        total_tax = 0
        ...
        try:
            ...
            return JsonResponse({"message": "discount code is not found."}, status=404)
        else:
            if coupon.used_by.count() >= coupon.max_limit_usage:
                status_code, message = 403, "uses limited"

            elif coupon.expiration_date and coupon.expiration_date < timezone.now():
                status_code, message = 403, "discount code is expire"

            elif user in coupon.used_by.all():
                status_code, message = 403, "This discount code uses before by you."
